...
contract CuyToken is IERC20, Pausable, CriptoCredit {
        ...
        returns (bool success)
    {
        require(
            to != address(0) &&
                (balances[msg.sender] - conditionedBalances[msg.sender]) >=
                value,
            INSUFFICIENT_FUNDS
        );
     ...
     */
    function shopPay(address to, uint256 value)
        public
        whenNotPaused
        returns (bool success)
    {
        require(
            to != address(0) && conditionedBalances[msg.sender] >= value,
            INSUFFICIENT_FUNDS
        );
    ...
    function transferFrom(
        ...
        require(
            spender != address(0) &&
                value <= (balances[from] - conditionedBalances[from]) &&
     ...
     */
    function balancesTransform(address from, uint256 value)
        public
        onlyAdmin
        whenNotPaused
        returns (bool success)
